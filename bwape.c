...
void bwa_sai2sam_pe_core(const char *prefix, char *const sai_prefix, char *const sai_prefix2, char *const fn_fa[2], pe_opt_t *popt, MPI_File fh)
{
	...
	sprintf(fn_sa[1], SAI_FILE_PATTERN, sai_prefix2, 2, rank);

	//check to see if we created both SAI files in one go
	if (fopen(fn_sa[1], "r") != NULL) {
		
		//we did, so the filename is correct
	} else {
	
		//else they were created separately, redefine the filename
		sprintf(fn_sa[1], SAI_FILE_PATTERN, sai_prefix2, 1, rank);
	}

	fprintf(stderr, "Proc %d: Found second SAI file - %s\n", rank, fn_sa[1]);
...
int bwa_sai2sam_pe(int argc, char *argv[])
{
	...
	if (optind + 4 > argc) {
		fprintf(stderr, "\n");
		fprintf(stderr, "Usage:   pBWA sampe -f <output.sam> [options] <prefix> <SAI_FILE_PREFIX1> <SAI_FILE_PREFIX2> <in1.fq> <in2.fq>\n\n");
		...
		fprintf(stderr, "       3. For the SAI prefixes, do NOT include the _1 and _2 generated by aln\n");
		fprintf(stderr, "          as sampe will auto-detect these.\n");
		fprintf(stderr, "\n");
		return 1;
	}
